...
point avvia_nave(int *DEB_porti_attraccati, int *DEB_porti_lasciati, int *DEB_porto_ultima_destinazione, int *indice_porto_attraccato){
...
richiesta esamina_porto(int* lotti_scartati, int *indice_porto_attraccato, int *reqlett, point posizione, int *spaziolibero, double *tempo_carico, merce_nave carico[], int *i_carico, int *indice_destinazione){
    richiesta r;
    double distanza;
...
void carica_dal_porto(richiesta r, point posizione, int *indice_destinazione, int *indice_porto_attraccato, int *lotti_scartati, int *spaziolibero, double *tempo_carico, int *i_carico, int *reqlett, merce_nave carico[]){
    double distanza;
    distanza = calcola_distanza(posizione, CAST_POSIZIONI_PORTI(vptr_shm_posizioni_porti)[r.mtype]);
    int noncaricare = 0;
    int j;
...
void attracco_e_scarico(int *spaziolibero, int *i_carico, double *tempo_carico, int *reqlett, int *indice_porto_attraccato, merce_nave carico[]){
    int j, datascarico;
...
void codice_simulazione(){
    ...
    posizione = avvia_nave(&DEB_porti_attraccati, &DEB_porti_lasciati, &DEB_porto_ultima_destinazione, &indice_porto_attraccato);
    while(1){
        /* Il primo do-while esegue la ricerca della prima richiesta da accettare,

         * in base alle risorse del porto di attracco. */
        sem_reserve(id_semaforo_mercato, indice_porto_attraccato);
        r = esamina_porto(&lotti_scartati, &indice_porto_attraccato, &reqlett, posizione, &spaziolibero, &tempo_carico,
        ...
        if(reqlett == MAX_REQ_LETTE){
            ...
            * del porto associato alla prima accettata. */
            carica_dal_porto(r, posizione, &indice_destinazione, &indice_porto_attraccato, &lotti_scartati, &spaziolibero, &tempo_carico,
        ...
        attracco_e_scarico(&spaziolibero, &i_carico, &tempo_carico, &reqlett, &indice_porto_attraccato, carico);
    }   
}
